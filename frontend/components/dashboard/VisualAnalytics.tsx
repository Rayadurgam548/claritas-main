...
export function VisualAnalytics({ analytics, simulation, timeline }: VisualAnalyticsProps) {
  ...
  const distributionData = [
    { name: t.safe, value: analytics.riskDistribution.safe },
    { name: t.moderate, value: analytics.riskDistribution.moderate },
    { name: t.risky, value: analytics.riskDistribution.risky },
  ];

  const categoryData = [
    { name: t.penalty, count: analytics.categories.penalty },
    { name: t.interest, count: analytics.categories.interest },
    { name: t.lock_in, count: analytics.categories.lockIn },
    { name: t.liability, count: analytics.categories.liability },
  ];
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="glass p-6 rounded-3xl border border-border/50 shadow-sm">
          <h3 className="text-sm font-bold flex items-center gap-2 mb-4">
            <Activity className="w-4 h-4 text-primary" /> {t.risk_dist}
          </h3>
          <div className="h-48">
          ...
          <div className="flex justify-center gap-4 mt-2">
            <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-success"></div><span className="text-xs">{t.safe}</span></div>
            <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-warning"></div><span className="text-xs">{t.moderate}</span></div>
            <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-danger"></div><span className="text-xs">{t.risky}</span></div>
          </div>
        </div>
        ...
        <div className="glass p-6 rounded-3xl border border-border/50 shadow-sm">
          <h3 className="text-sm font-bold flex items-center gap-2 mb-4">
            <AlertTriangle className="w-4 h-4 text-warning" /> {t.clause_cats}
          </h3>
          <div className="h-52">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="glass p-6 rounded-3xl border border-danger/30 shadow-sm relative overflow-hidden group">
          ...
          <h3 className="text-sm font-bold flex items-center gap-2 mb-4">
            <DollarSign className="w-4 h-4 text-danger" /> {t.fin_simulator}
          </h3>
          <div className="space-y-4 relative z-10">
            <div>
              <p className="text-xs text-muted-foreground uppercase tracking-wide font-semibold mb-1">{t.loss_range}</p>
              <p className="text-xl font-bold text-danger">{simulation.financialLoss}</p>
            </div>
            <div className="pt-2 border-t border-border/50">
              <p className="text-xs text-muted-foreground uppercase tracking-wide font-semibold mb-1">{t.worst_case}</p>
        ...
        <div className="glass p-6 rounded-3xl border border-border/50 shadow-sm overflow-hidden">
          <h3 className="text-sm font-bold flex items-center gap-2 mb-4">
            <Calendar className="w-4 h-4 text-primary" /> {t.timelines}
          </h3>
          <div className="space-y-4 max-h-40 overflow-y-auto pr-2 custom-scrollbar relative before:absolute before:inset-0 before:ml-[9px] before:h-full before:w-0.5 before:bg-border">
            {timeline.length > 0 ? timeline.map((t, i) => (
              ...
              <p className="text-sm text-muted-foreground pl-8">{t.no_timelines}</p>
            )}
          </div>
        </div>
